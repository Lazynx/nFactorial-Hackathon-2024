...
class ExcelService {
  ...
  private async sendToGemini(data: any): Promise<string> {
    ...
    желательно брать людей у которых уже есть опыт в программирование вебсайтов (уже использовал какие либо веб-фреймворки)
    отвечай мне с причинами почему да или почему нет коротко.
    ${JSON.stringify(data)}
    `
